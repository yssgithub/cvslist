...
public class DateChooser extends JPanel {
    ...
    private Boolean curCompnentFlag;//true-start false-end
    private JComponent otherShowDate;//the other compnent
...
//    public void register(final JComponent showDate) {
    public void register(final JComponent showDate, Boolean curCompnentFlag, JComponent otherShowDate) {
        this.curCompnentFlag = curCompnentFlag;
        this.otherShowDate = otherShowDate;
...
//                Date selectDate = select.getTime();
                Date selectDate = sdf.parse(sdf.format(select.getTime()));
                Date date1 = sdf.parse(((JTextField)showDate).getText());
                Date date2 = sdf.parse(((JTextField)otherShowDate).getText());
                if(curCompnentFlag && selectDate.after(date2)){
                    //不能比另一个晚
                    JOptionPane.showMessageDialog(null, "不能晚于结束时间");
                    return;
                }else if(!curCompnentFlag && selectDate.before(date2)){
                    //不能比另一个早
                    JOptionPane.showMessageDialog(null, "不能早于开始时间");
                    return;
                }
    ...
    private class LabelManager {
        private java.util.List<MyLabel> list;
